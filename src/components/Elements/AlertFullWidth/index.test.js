...
import AlertFullWidthContainer from './index'
...
function setup() {
    ...
    const enzymeWrapper = mount(<AlertFullWidthContainer {...props} />)
...
describe('AlertFullWidthContainer', () => {

    it('Should render itself correctly', () => {
        const { enzymeWrapper } = setup()

        expect(enzymeWrapper.find('AlertFullWidth').exists()).toBe(true)
        expect(enzymeWrapper.find('section').exists()).toBe(true)
        expect(enzymeWrapper.find('section').hasClass('alert-warning-fullwidth')).toBe(true)
        ...
        expect(enzymeWrapper.find('.dismiss').exists()).toBe(true)
    })
    ...
    const setAlertLevel = (event) => {
        it(`Should show correct icon for alert level ${event.alert.level}`, () => {
            ...
            expect(enzymeWrapper.find('section').hasClass(`alert-${event.alert.level}-fullwidth`)).toBe(true)
    ...
    describe('onClick()', () => {
        ...
        const runOnClickUpdateStateTest = (event) => {
			it(`should update isHidden state to ${event.target.result}`, () => {
                ...
                expect(enzymeWrapper.find('AlertFullWidth').exists()).toBe(event.target.result)
			})
		}
